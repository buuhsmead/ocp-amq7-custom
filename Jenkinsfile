...
// 								openshift.apply(amqSecretSelector) // Patch the object on the server
							...
							if (!customAMQ7BcSelector.exists()) {
								customAMQ7Build = openshift.newBuild("amq-broker:7.5~https://github.com/buuhsmead/ocp-amq7-custom.git", '--name=amq7-custom').narrow("bc").related('builds')
							}
							else {
